...
public class SalesReporter {
	...
	// 배열의 총 합을 계산하여 반환
	public double calculateTotalRevenue(double[] salesData) {
		double sum = 0;
		
		for(double x : salesData) {
			sum += x;
		}
	...
	// 매출별 10%씩 뗀 금액이 얼만지
	public double calculateTax(double[] salesData) {
		return calculateTotalRevenue(salesData) * 1.1;
	}
