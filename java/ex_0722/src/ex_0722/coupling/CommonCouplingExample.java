...
public class CommonCouplingExample {
	...
	// 가격 계산 모듈
	// ModuleA가 전역변수를 바꾼 파급효과가 ModuleB에 그대로 전달되어 예기치 않은 문제가 발생할 수 있다.
	// 프로그램 전체에서 어느 모듈이 언제 전역 변수 값을 바꿨는지 추적하기가 힘들어 디버깅이 어려워짐.(전역변수를 트래킹해서 전부 확인해야함)
	...
	static class ModuleB{
		public void printPrice(int price) {
			...
			System.out.println("적용된 할인률 후 가격 : " + finalPrice);
		}
	}
