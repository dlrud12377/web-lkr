...
// 대상 : final이 붙은 필드, @NonNull이 붙은 필드
public class Chef {
	...
	// knife 객체 수동생성 : knife = new KitchenKnife();
	// -> Autowired 이용해서 필드 주입을 통해 객체 자동생성(코드가 유연해짐)
...
//	@Autowired
	...
	// 2. 세터(setter) 주입
	// setter 메서트를 통해 의존성을 주입하는 방식
	
	// 장점 : 의존성을 나중에 변경할 수 있다.
	// + 선택적으로 필요한 객체를 주입할 때 사용할 수 있다.
	
	// 단점 : Setter를 호출하지 않으면 객체가 정상적으로 동작하지 않을 수 있다.
...
//		this.pot = pot;
//	}
	// 일일히 추가가 귀찮음 -> 롬복을 이용하여 진행
	
	// Chef chef = new Chef(new Knife()); 가 자동 실행된다.
	
	// 정리
	
	// 필드 주입 : 객체 생성 -> 필드에 주입
	// setter 주입 : 객체 생성 -> Setter 호출 -> 주입
	// 생성자 주입 : 객체 생성 -> 객체 생성하면서(생성자 호출) -> 동시에 주입
